...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ─── modalFactory ────────────────
    function modalFactory(prefix, endpoint, ps_name) {
        ...
        function open(edit_status = false) {
        ...
        submitBtn.addEventListener('click', () => {
            ...
            fetch(endpoint, {
                ...
                .then(json => {
                    if (json.success) {
                        close();
                        showToast(`Bookmark ${ps_name} !`, 'success');
                    } else {
    ...
    //         // 1️⃣ show the modal (which also resets by default)
    //         editModal.open();
    ...
    //         const id = delBtn.dataset.id || card.dataset.bmId;
        ...
        try {
            ...
            if (json.results.length) {
                ...
                json.results.forEach(bm => {
                    ...
                    card.className = 'card card-effects p-4 md:p-6 relative cursor-pointer';
                    ...
                    card.innerHTML = `
                        ...
                        <h3 class="mb-2 font-semibold text-lg md:text-2xl truncate">${bm.name}</h3>
                        ...
                        <div class="tags mt-3 flex flex-wrap gap-1 md:gap-2 max-h-[1.5rem] md:max-h-[2rem] overflow-hidden">
                            <span class="text-[10px] sm:text-xs md:text-sm tag">${bm.platform}</span>
                            ${tagsHtml}
                            ${moreHtml}
                        </div>
                        `;
